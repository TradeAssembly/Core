...
fn tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        tool("tradeassembly.backtest.process", "Process Backtest Queue", "Claim and process one durable local BacktestRun worker delivery.", object_schema([("worker", string_schema("Stable worker identity.", Some("local-backtest-worker"))), ("idempotency_key", string_schema("Command idempotency key.", None)), ("db", db_property())], ["idempotency_key"]), false),
